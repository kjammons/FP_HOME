...
function quickselect(array5, k2, left2 = 0, right2 = Infinity, compare) {
  ...
  while (right2 > left2) {
    ...
    if (compare(array5[right2], t) > 0)
      swap(array5, left2, right2);
    while (i < j) {
      swap(array5, i, j), ++i, --j;
      while (compare(array5[i], t) < 0)
        ++i;
      while (compare(array5[j], t) > 0)
        --j;
    }
    if (compare(array5[left2], t) === 0)
      swap(array5, left2, j);
    else
      ++j, swap(array5, j, right2);
    if (j <= k2)
      left2 = j + 1;
    if (k2 <= j)
      right2 = j - 1;
...
function color2(format2) {
  var m3, l;
  format2 = (format2 + "").trim().toLowerCase();
  return (m3 = reHex2.exec(format2)) ? (l = m3[1].length, m3 = parseInt(m3[1], 16), l === 6 ? rgbn2(m3) : l === 3 ? new Rgb2(m3 >> 8 & 15 | m3 >> 4 & 240, m3 >> 4 & 15 | m3 & 240, (m3 & 15) << 4 | m3 & 15, 1) : l === 8 ? rgba2(m3 >> 24 & 255, m3 >> 16 & 255, m3 >> 8 & 255, (m3 & 255) / 255) : l === 4 ? rgba2(m3 >> 12 & 15 | m3 >> 8 & 240, m3 >> 8 & 15 | m3 >> 4 & 240, m3 >> 4 & 15 | m3 & 240, ((m3 & 15) << 4 | m3 & 15) / 255) : null) : (m3 = reRgbInteger2.exec(format2)) ? new Rgb2(m3[1], m3[2], m3[3], 1) : (m3 = reRgbPercent2.exec(format2)) ? new Rgb2(m3[1] * 255 / 100, m3[2] * 255 / 100, m3[3] * 255 / 100, 1) : (m3 = reRgbaInteger2.exec(format2)) ? rgba2(m3[1], m3[2], m3[3], m3[4]) : (m3 = reRgbaPercent2.exec(format2)) ? rgba2(m3[1] * 255 / 100, m3[2] * 255 / 100, m3[3] * 255 / 100, m3[4]) : (m3 = reHslPercent2.exec(format2)) ? hsla2(m3[1], m3[2] / 100, m3[3] / 100, 1) : (m3 = reHslaPercent2.exec(format2)) ? hsla2(m3[1], m3[2] / 100, m3[3] / 100, m3[4]) : named2.hasOwnProperty(format2) ? rgbn2(named2[format2]) : format2 === "transparent" ? new Rgb2(NaN, NaN, NaN, 0) : null;
}
function rgbn2(n) {
...
define_default2(Cubehelix2, cubehelix3, extend2(Color2, {
  ...
  rgb() {
    ...
    return new Rgb2(
      255 * (l + a4 * (A2 * cosh2 + B2 * sinh2)),
      255 * (l + a4 * (C2 * cosh2 + D2 * sinh2)),
      255 * (l + a4 * (E2 * cosh2)),
      this.opacity
    );
  }
...
function interpolate_default(a4, b) {
  var c6;
  return (typeof b === "number" ? number_default : b instanceof color2 ? rgb_default : (c6 = color2(b)) ? (b = c6, rgb_default) : string_default)(a4, b);
}
...
function brush(dim) {
  ...
  function started(event) {
    ...
    if (!filter4.apply(this, arguments))
      return;
    var that = this, type2 = event.target.__data__.type, mode3 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent4 = state.extent, selection2 = state.selection, W = extent4[0][0], w0, w1, N = extent4[0][1], n0, n1, E3 = extent4[1][0], e0, e1, S = extent4[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
...
function tickStep2(start2, stop, count4) {
  stop = +stop, start2 = +start2, count4 = +count4;
  const reverse3 = stop < start2, inc2 = reverse3 ? tickIncrement2(stop, start2, count4) : tickIncrement2(start2, stop, count4);
  return (reverse3 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}
...
function formatDate(date2) {
  var hours = date2.getUTCHours(), minutes = date2.getUTCMinutes(), seconds3 = date2.getUTCSeconds(), milliseconds3 = date2.getUTCMilliseconds();
  return isNaN(date2) ? "Invalid Date" : formatYear(date2.getUTCFullYear(), 4) + "-" + pad(date2.getUTCMonth() + 1, 2) + "-" + pad(date2.getUTCDate(), 2) + (milliseconds3 ? "T" + pad(hours, 2) + ":" + pad(minutes, 2) + ":" + pad(seconds3, 2) + "." + pad(milliseconds3, 3) + "Z" : seconds3 ? "T" + pad(hours, 2) + ":" + pad(minutes, 2) + ":" + pad(seconds3, 2) + "Z" : minutes || hours ? "T" + pad(hours, 2) + ":" + pad(minutes, 2) + "Z" : "");
}
...
function formatDate2(date2) {
  var hours = date2.getUTCHours(), minutes = date2.getUTCMinutes(), seconds3 = date2.getUTCSeconds(), milliseconds3 = date2.getUTCMilliseconds();
  return isNaN(date2) ? "Invalid Date" : formatYear2(date2.getUTCFullYear(), 4) + "-" + pad2(date2.getUTCMonth() + 1, 2) + "-" + pad2(date2.getUTCDate(), 2) + (milliseconds3 ? "T" + pad2(hours, 2) + ":" + pad2(minutes, 2) + ":" + pad2(seconds3, 2) + "." + pad2(milliseconds3, 3) + "Z" : seconds3 ? "T" + pad2(hours, 2) + ":" + pad2(minutes, 2) + ":" + pad2(seconds3, 2) + "Z" : minutes || hours ? "T" + pad2(hours, 2) + ":" + pad2(minutes, 2) + "Z" : "");
}
...
function circle_default2(radius) {
  var cr = cos2(radius), delta = 2 * radians3, smallRadius = cr > 0, notHemisphere = abs3(cr) > epsilon7;
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function ticks3(start2, stop, count4) {
    ...
    const ticks4 = interval3 ? interval3.range(start2, +stop + 1) : [];
...
function timeInterval2(floori, offseti, count4, field) {
  function interval3(date2) {
    return floori(date2 = arguments.length === 0 ? /* @__PURE__ */ new Date() : /* @__PURE__ */ new Date(+date2)), date2;
  }
  interval3.floor = (date2) => {
...
var utcDay2 = timeInterval2((date2) => {
  date2.setUTCHours(0, 0, 0, 0);
}, (date2, step) => {
  date2.setUTCDate(date2.getUTCDate() + step);
}, (start2, end) => {
...
var utcYear2 = timeInterval2((date2) => {
  date2.setUTCMonth(0, 1);
  date2.setUTCHours(0, 0, 0, 0);
}, (date2, step) => {
  date2.setUTCFullYear(date2.getUTCFullYear() + step);
...
function ticker2(year, month, week, day, hour, minute) {
  ...
  function tickInterval(start2, stop, count4) {
    ...
    if (i === tickIntervals.length)
      return year.every(tickStep2(start2 / durationYear2, stop / durationYear2, count4));
    if (i === 0)
      return millisecond2.every(Math.max(tickStep2(start2, stop, count4), 1));
    const [t, step] = tickIntervals[target / tickIntervals[i - 1][2] < tickIntervals[i][2] / target ? i - 1 : i];
    return t.every(step);
  }
  return [ticks3, tickInterval];
...
function formatUTCDayOfYear(d, p) {
  return pad3(1 + utcDay2.count(utcYear2(d), d), p, 3);
}
...
function formatUTCWeekNumberSunday(d, p) {
  return pad3(utcSunday2.count(utcYear2(d) - 1, d), p, 2);
}
function UTCdISO(d) {
  var day = d.getUTCDay();
...
function formatUTCWeekNumberISO(d, p) {
  d = UTCdISO(d);
  return pad3(utcThursday2.count(utcYear2(d), d) + (utcYear2(d).getUTCDay() === 4), p, 2);
}
function formatUTCWeekdayNumberSunday(d) {
  return d.getUTCDay();
}
function formatUTCWeekNumberMonday(d, p) {
  return pad3(utcMonday2.count(utcYear2(d) - 1, d), p, 2);
}
...
function calendar(ticks3, tickInterval, year, month, week, day, hour, minute, second3, format2) {
  ...
  scale2.copy = function() {
    return copy(scale2, calendar(ticks3, tickInterval, year, month, week, day, hour, minute, second3, format2));
  };
  return scale2;
}
function time() {
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x22 = 1, s2 = 1, t05, t15, t23, k10, k21, interpolator = identity4, transform2, clamp = false, unknown;
  function scale2(x4) {
...
var cubehelix_default2 = cubehelixLong(cubehelix3(300, 0.5, 0), cubehelix3(-240, 0.5, 1));

// node_modules/d3-scale-chromatic/src/sequential-multi/rainbow.js
var warm = cubehelixLong(cubehelix3(-100, 0.75, 0.35), cubehelix3(80, 1.5, 0.8));
var cool = cubehelixLong(cubehelix3(260, 0.75, 0.35), cubehelix3(80, 1.5, 0.8));
var c3 = cubehelix3();
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon8))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon8 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min4(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t05, t15;
      if (rp > epsilon8) {
